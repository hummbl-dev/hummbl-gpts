...
function getRecentCommits(count = 10) {
  return sh(`git log --oneline -n ${count}`, 'No git history available.');
}
...
function generateLivingArtifacts(manifest, specs, meta) {
  ...
  lines.push(`- Generated at (UTC): ${meta.now}`);
  lines.push(`- Branch: ${meta.branch}`);
  lines.push(`- HEAD at generation start: ${meta.head}`);
...
function generateActiveChangelog(meta) {
  ...
  lines.push('');
  lines.push(`- Refreshed at (UTC): ${meta.now}`);
  lines.push(`- HEAD at generation start: ${meta.head}`);
  lines.push('');
